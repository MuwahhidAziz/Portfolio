...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if y in self.crossword.neighbors(x):
            ...
            for word in self.domains[x]:
                if not any(word[i] == compare[j] for compare in self.domains[y]):
                    deletions.append(word)
            if deletions:
                changed = True
                for word in deletions:
    ...
    def ac3(self, arcs=None):
        ...
        """
        queue = arcs if arcs is not None else [(v1, v2) for v2 in self.domains for v1 in self.domains if v1 != v2 and self.crossword.overlaps[v1, v2] is not None]
        while queue:
            x, y = queue.pop(0)
            if self.revise(x, y):
    ...
    def select_unassigned_variable(self, assignment):
        ...
        return values.
        """
        unassigned = sorted([v for v in self.crossword.variables if v not in assignment], key=lambda x: len(self.domains[x]))
        minlen = len(self.domains[unassigned[0]])
        ties = [v for v in unassigned if len(self.domains[v]) == minlen]
        if len(ties) == 1:
            return ties[0]
        return max(ties, key=lambda v: sum(1 for other in self.crossword.variables if other != v and self.crossword.overlaps[v, other] is not None))
